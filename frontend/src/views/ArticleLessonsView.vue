...
async function createLesson() {
    ...
    payload.append('blocks', JSON.stringify(form.value.blocks.map((block) => ({ id: block.id, type: block.type, content: block.content ?? '', replace_image: Boolean(block.replaceImage) }))));
    ...
    try {
        ...
        const { data } = editingLesson.value
            ? await api.post(`/article-lessons/${editingLesson.value.id}`, payload)
            : await api.post('/article-lessons', payload);
        ...
        errorMessage.value = error.response?.data?.errors
            ...
            : error.response?.data?.message ?? 'Не удалось опубликовать урок.';
    }
    finally {
        saving.value = false;
    }
...
function openEdit(lesson) {
    ...
    form.value = {
        ...
        blocks: lesson.blocks?.length
            ? lesson.blocks.map((block) => ({ id: block.id, type: block.type, content: block.content ?? '', file: null, preview: block.image_path ?? '' }))
...
<template>
  <section class="grid gap-6">
    ...
    <Teleport to="body">
      <div v-if="selectedLesson" class="fixed inset-0 z-[110] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-5" @mousedown.self="selectedLesson = null">
        <article class="glass-panel max-h-[92vh] w-full overflow-y-auto rounded-t-[28px] p-5 sm:max-w-3xl sm:rounded-[28px] sm:p-8" role="dialog" aria-modal="true" :aria-label="selectedLesson.title">
          ...
          <div v-if="selectedLesson.blocks?.length" class="grid gap-6"><template v-for="block in selectedLesson.blocks" :key="block.id"><div v-if="block.type === 'text'" class="whitespace-pre-wrap text-[15px] leading-7 text-on-surface">{{ block.content }}</div><img v-else class="w-full rounded-2xl object-cover" :src="block.image_path" alt="Иллюстрация к уроку" /></template></div>
      ...
      <div v-if="createModalOpen" class="fixed inset-0 z-[120] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-5" @mousedown.self="closeCreateModal">
        <form class="glass-panel max-h-[92vh] w-full overflow-y-auto rounded-t-[28px] p-5 sm:max-w-3xl sm:rounded-[28px] sm:p-7" role="dialog" aria-modal="true" aria-labelledby="lesson-create-title" @submit.prevent="createLesson">
          ...
          <div class="grid gap-4"><label class="grid gap-2 text-sm font-bold text-on-muted">Название<input v-model.trim="form.title" class="rounded-2xl border border-white/10 bg-surface-low px-4 py-3 text-on-surface outline-none focus:border-primary/50" required maxlength="255" /></label><label class="grid gap-2 text-sm font-bold text-on-muted">Краткое описание<textarea v-model.trim="form.excerpt" class="min-h-24 rounded-2xl border border-white/10 bg-surface-low px-4 py-3 text-on-surface outline-none focus:border-primary/50" required maxlength="500" /></label><fieldset class="grid gap-2"><legend class="text-sm font-bold text-on-muted">Доступ к уроку</legend><div class="grid grid-cols-2 rounded-2xl border border-white/10 bg-surface-low p-1"><button v-for="option in [{ value: 'free', label: 'Бесплатный' }, { value: 'paid', label: 'Платный' }]" :key="option.value" class="rounded-xl px-4 py-3 text-sm font-extrabold transition" :class="form.access_level === option.value ? 'bg-primary text-[#470382]' : 'text-on-muted hover:bg-white/5'" type="button" @click="form.access_level = option.value">{{ option.label }}</button></div></fieldset></div>
          <div class="mt-6 border-t border-white/10 pt-5"><div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h4 class="font-extrabold">Содержание урока</h4><p class="mt-1 text-sm text-on-muted">Добавляйте блоки в нужном порядке: текст, фото, затем снова текст.</p></div><div class="flex gap-2"><button class="inline-flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-sm font-bold text-on-muted" type="button" @click="addTextBlock"><span class="material-symbols-outlined text-[18px]">text_fields</span>Текст</button><button class="inline-flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-sm font-bold text-on-muted" type="button" @click="addImageBlock"><span class="material-symbols-outlined text-[18px]">image</span>Фото</button></div></div>
            <div class="mt-4 grid gap-4"><article v-for="(block, index) in form.blocks" :key="index" class="rounded-2xl border border-white/10 bg-surface-low p-4"><div class="mb-3 flex items-center justify-between"><span class="text-sm font-extrabold text-primary">{{ block.type === 'text' ? 'Текстовый блок' : 'Блок с фото' }}</span><button v-if="form.blocks.length > 1" class="grid h-8 w-8 place-items-center rounded-lg text-on-muted hover:bg-red-500/15 hover:text-red-200" type="button" aria-label="Удалить блок" @click="removeBlock(index)"><span class="material-symbols-outlined text-[18px]">delete</span></button></div><textarea v-if="block.type === 'text'" v-model="block.content" class="min-h-36 w-full rounded-xl border border-white/10 bg-surface-container px-4 py-3 text-on-surface outline-none focus:border-primary/50" placeholder="Введите текст этого фрагмента урока" required /><label v-else class="grid cursor-pointer gap-3"><img v-if="block.preview" class="max-h-80 w-full rounded-xl object-cover" :src="block.preview" alt="Предпросмотр фото" /><span v-else class="grid aspect-video place-items-center rounded-xl border border-dashed border-white/15 bg-surface-container text-on-muted"><span class="material-symbols-outlined text-4xl">add_photo_alternate</span></span><span class="truncate text-sm font-semibold text-primary">{{ block.file?.name ?? 'Выбрать фото с устройства' }}</span><input class="sr-only" required type="file" accept="image/jpeg,image/png,image/webp" @change="selectBlockImage($event, block)" /></label></article></div>
          </div>
          <p v-if="errorMessage" class="mt-5 rounded-2xl border border-red-400/25 bg-red-500/10 p-3 text-sm font-semibold text-red-200">{{ errorMessage }}</p><div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button class="rounded-2xl border border-white/10 px-5 py-3 text-sm font-bold text-on-muted" type="button" :disabled="saving" @click="closeCreateModal">Отмена</button><button class="rounded-2xl bg-gradient-to-br from-primary-container to-primary-strong px-6 py-3 text-sm font-extrabold text-white disabled:opacity-60" type="submit" :disabled="saving">{{ saving ? 'Сохраняем...' : editingLesson ? 'Сохранить изменения' : 'Опубликовать урок' }}</button></div>
        </form>
      </div>
    </Teleport>
  </section>
